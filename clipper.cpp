...
void ClipperBase::Reset()
{
  ...
  sort(m_MinimaList.begin(), m_MinimaList.end(), LocMinSorter());
...
bool Clipper::FixupIntersectionOrder()
{
  ...
  CopyAELToSEL();
  sort(m_IntersectList.begin(), m_IntersectList.end(), IntersectListSort);
...
ostream& operator <<(ostream &s, const IntPoint &p)
{
  s << "(" << p.X << "," << p.Y << ")";
  return s;
}
...
ostream& operator <<(ostream &s, const Paths &p)
{
  for (Paths::size_type i = 0; i < p.size(); i++)
    s << p[i];
  s << "\n";
